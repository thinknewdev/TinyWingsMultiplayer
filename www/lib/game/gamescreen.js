...
.defines(function(){
	...
	enemy.draw = function()
	{
		...
		ctx.save();
		//
		ctx.translate(enemy.coord.x-player.coord.x,enemy.coord.y);
	...
	enemy.enterFrame = function()
	{
		...
		if(player.startMoving){
			...
			var setX = enemy.coord.x + 50
			var aheadY = terrain.funct(setX);
			if(HUD.timer>0){
				//ig.log("MOUSE IS DOWN");

				if(enemy.touchGround2){
					if(enemy.vel.y>0){
						enemy.vel.y += 0.3; 
						//player.vel.x += 0.05; 
					}else{
				...
				if(HUD.timer>0 && enemy.touchGround2){
					if(enemy.vel.y<0 && enemy.vel.x<3){
						enemy.vel.x += 0.05;
					}
				}
				if(HUD.timer<=0 && !enemy.touchGround2){
			...
			// Move coords
			enemy.coord.x += enemy.vel.x;
			var terrY = terrain.funct(enemy.coord.x);
			if(enemy.touchGround3){
				enemy.coord.y += terrY;
			}else{
				enemy.coord.y += enemy.vel.y;
			}	
			// Terrain Update
			
			//terrain.updateX(enemy.coord.x);
			
			// Correct coords
			enemy.touchGround = (   ( enemy.vel.y>0 && enemy.coord.y>terrY-2 )
		                       || ( enemy.vel.y<0 && enemy.coord.y>terrY-0.5 )
		                       );
		 	//player.touchGround3 = player.coord.y>terrY-100;
		 	enemy.touchGround2 = enemy.coord.y>terrY-5;
		 	//enemy.touchGround = enemy.coord.y>terrY;
			if(enemy.touchGround)
			{
				//ig.log("touching");
				enemy.coord.y = terrY;
				// Slope & Projection
				var terrSlope = terrain.functDiff(enemy.coord.x);
				var terrLength = Math.sqrt(1*1+terrSlope*terrSlope);
				var dotProduct = player.vel.x*1 + enemy.vel.y*terrSlope;
				dotProduct = dotProduct/terrLength;
				enemy.vel.x = dotProduct/Math.sqrt(1+terrSlope*terrSlope);
				if(enemy.vel.x<0.1){
					enemy.vel.x=0.1;
				}
				enemy.vel.y = enemy.vel.x*terrSlope;
				enemy.vel.x*=0.995;
				enemy.vel.y*=0.995;
			}else{
				//player.coord.y += player.vel.y;
			...
			enemy.rotation = Math.atan2(enemy.vel.y,enemy.vel.x);
			if(enemy.rotation>Math.PI*0.3){
				enemy.rotation*=3;
				enemy.rotation+=Math.PI*0.3;
				enemy.rotation*=0.25;
			}
			...
			//terrain = ig.game.spawnEntity(Terrain, 0,0);
			  ...
			  /*
			   $.ajax({
					...
					jsonp: 'callback',
					url: 'http://208.89.208.52/GetToken/com.thinknew.tinywings',
			  ...
			  /*
			  $.jsonp({
				  url: "http://208.89.208.52/GetToken/com.thinknew.tinywings?callback=callback",
